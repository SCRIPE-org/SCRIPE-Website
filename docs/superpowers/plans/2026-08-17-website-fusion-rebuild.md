...
- [ ] **Step 2:** Per the Hero integration contract (Task 13 report §8, amended by controller ruling 2026-08-18): background plate replaces `campus.jpg` as the rig's base `next/image` (priority); midground cutout + bokeh foreground render as sibling layers of `.hero-rig` (children of `.hero-stage`) each driven by its own depth-scaled GSAP tween (mid ×1.15, near ×1.4) — sibling structure ratified over nesting to avoid compounding parent/child transforms; finale plate crossfades in across scrub 0.76→0.86.
