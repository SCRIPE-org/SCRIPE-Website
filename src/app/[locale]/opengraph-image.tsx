 ...
 *
 * Task G6: since the above is a structural Satori limitation, not a missing
 * font, the `ar` locale's real social-share card moved off this route
 * entirely — `src/lib/seo/metadata.ts`'s `pageMetadata` now points `ar`'s
 * `openGraph.images`/`twitter.images` at a static, pre-rendered PNG
 * (`public/og/og-ar.png`, built by `scripts/build-og-ar-card.mjs`, which
 * renders this same card design in headless Chromium — a real browser
 * shapes Arabic correctly). This route's `ar` output (still generated by
 * `generateStaticParams` below, still reachable at `/ar/opengraph-image`)
 * is deliberately left in place but unlinked: nothing in the site's
 * metadata references it anymore, so its English-fallback tagline is inert
 * rather than user-facing. Deleting or redirecting the route was considered
 * and rejected as unnecessary churn for a route nothing points at.
 */
